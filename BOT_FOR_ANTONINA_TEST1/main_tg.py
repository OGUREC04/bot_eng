...
def information_of_all_student(update: Update, context: CallbackContext):
    ...
    try:
        key = str(update.effective_user.id) + ' is_admin'
        if context.bot_data[key]:
            table = pt.PrettyTable(['id', 'nick_name', 'user_name', 'user_surname'])
            table.align['id'] = 'l'
            table.align['nick_name'] = 'l'
            table.align['user_name'] = 'l'
            table.align['user_surname'] = 'l'
            cursor.execute("""
                    SELECT  id, nick_name, user_name, user_surname FROM users;
                    """)
            data = cursor.fetchall()
            print(data)
            for id, nick_name, user_name, user_surname in data:
                  table.add_row([id, f'{nick_name:}', f'{user_name:}', f'{user_surname:}'])
...
def student_achivment_second(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if context.bot_data[key]:
            ...
            ### get task1
            cursor.execute("""
                          SELECT  result, task, errors, date_do_task FROM random_task WHERE fk_random_task_user = (%s);
                          """, (user_id,))
            result_random_task = cursor.fetchall()
            print(result_random_task)
            ### даем имена колонкам
            with open("результаты_добровольного_задания.csv", "w") as csv_file:
            ...
            for item in result_random_task:
                ...
                errors = item[2]
                date = item[3]
                with open("результаты_добровольного_задания.csv", "a", encoding='utf-8',newline='') as csv_file:
            ...
            ### отправляем задание
            doc = open('результаты_добровольного_задания.csv', 'rb')
            context.bot.send_document(chat_id=ID, document=doc)
            doc.close()
            os.remove('результаты_добровольного_задания.csv')
            ### get task2
            cursor.execute("""
                                SELECT  result, task, errors, attemps, date_do_task FROM time_random_task WHERE fk_random_task_user = (%s);
                                """, (user_id,))
            time_result_random_task = cursor.fetchall()
            print(time_result_random_task)
            ###
            ### даем имена колонкам
            with open("результаты_обязательного_задания.csv", "w") as csv_file:
                csv_writer = csv.writer(csv_file)
                csv_writer.writerow(
                    (
                        'result',
            ...
            for item in time_result_random_task:
                ...
                attemps = item[3]
                date = item[4]
                with open("результаты_обязательного_задания.csv", "a", encoding='utf-8', newline='') as csv_file:
                    csv_writer = csv.writer(csv_file, delimiter=",",
                                            lineterminator="\r")
                    csv_writer.writerow(
                        (
            ...
            ### отправляем задание
            doc = open('результаты_обязательного_задания.csv', 'rb')
            context.bot.send_document(chat_id=ID, document=doc,reply_markup=markup_key)
            doc.close()
            os.remove('результаты_обязательного_задания.csv')
...
def student_statistic(update: Update, context: CallbackContext):
    ...
    # cursor = connection.cursor()
    # cursor.execute("""
    #                   SELECT  result, task, errors, attemps, date_do_task FROM time_random_task WHERE fk_random_task_user = 1;
    #                   """)
    # time_result_random_task = cursor.fetchall()
    # print(time_result_random_task)
    # df = DataFrame(time_result_random_task, columns=['result', 'task', 'errors', 'attemps', 'date_do_task'])
    # df = df.astype({'date_do_task': np.datetime64})
    # df['result'] = df['result'].replace("/", '', regex=True)
    # df['result'] = df['result'].replace('', 0, regex=True)
    # df = df.astype({'result': np.int64})
    # datatypes = df.dtypes
    # print(datatypes)
    # print(df)
