...
STUDENT_ACHIVMENT_SEOCND = range(1)
...
def administrator(update: Update, context: CallbackContext):
    ...
    context.bot.send_message(chat_id=ID, text="Введите пароль админа", reply_markup=markup_key)
...
def administrator_first(update: Update, context: CallbackContext):
    ...
    if update.message.text == '4321':
        ...
        context.bot.send_message(chat_id=ID, text="дарова админ")
...
def administrator_main(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if context.bot_data[key]:
            ...
            markup_key = ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True, resize_keyboard=True)
            context.bot.send_message(chat_id=ID, text="Выбирет варианты предлагаемые клавиатурой", reply_markup=markup_key)
            return ConversationHandler.END
        else:
...
def student_achivment(update: Update, context: CallbackContext):
    ...
    try:
        key = str(update.effective_user.id) + ' is_admin'
        if context.bot_data[key]:
            context.bot.send_message(chat_id=ID, text="Напишите никнейм ученика")
        ...
        if context.bot_data[key]:
            ...
            if messege in users_info:
                ...
                try:
                    ...
                    ### даем имена колонкам
                    with open(f"результаты_добровольного_задания_{update.effective_user.id}.csv", "w") as csv_file:
                        csv_writer = csv.writer(csv_file)
                        csv_writer.writerow(
                            (
                                'unit',
                    ...
                    ### записываем данные
                    for item in result_random_task:
                        unit = item[0]
                        errors = item[1]
                        date = item[2]
                        number_of_tasks = item[3]
                        number_of_right_answer = item[4]
                        with open(f"результаты_добровольного_задания_{update.effective_user.id}.csv", "a", encoding='utf-8',newline='') as csv_file:
                            csv_writer = csv.writer(csv_file, delimiter = ",",
                                         lineterminator="\r")
                            csv_writer.writerow(
                                (
                    ...
                    ### отправляем задание
                    doc = open(f'результаты_добровольного_задания_{update.effective_user.id}.csv', 'rb')
                    context.bot.send_document(chat_id=ID, document=doc)
                    doc.close()
                    os.remove(f'результаты_добровольного_задания_{update.effective_user.id}.csv')
                except Exception as _ex:
                    context.bot.send_document(chat_id=ID, document='Невозможно сформировать, возможно ученик не выполнял задания')
                    print("[INFO] Error while working with PostgreSQL", _ex)
                ### get task2
                try:
                    cursor.execute("""
                    ...
                    for item in time_result_random_task:
                        ...
                        attemps = item[2]
                        date = item[3]
                        number_of_tasks = item[4]
                        number_of_right_answer  = item[5]
                        with open(f"результаты_обязательного_задания_{update.effective_user.id}.csv", "a", encoding='utf-8', newline='') as csv_file:
                            csv_writer = csv.writer(csv_file, delimiter=",",
                                                    lineterminator="\r")
                            csv_writer.writerow(
                                (
                    ...
                    context.bot.send_document(chat_id=ID,
                                              document='Невозможно сформировать, возможно ученик не выполнял задания',reply_markup=markup_key)
...
def student_statistic(update: Update, context: CallbackContext):
    ...
    # random
    cursor.execute("""
                      SELECT task, errors,  date_do_task, number_of_tasks, number_of_right_answer FROM random_task WHERE fk_random_task_user = 14;
                      """)
    result_random_task = cursor.fetchall()
    print(result_random_task)
    # time
    cursor.execute("""
                      SELECT task, errors, attemps, date_do_task, number_of_tasks, number_of_right_answer FROM time_random_task WHERE fk_random_task_user = 14;
                      """)
    time_result_random_task = cursor.fetchall()
    print(time_result_random_task)
    # statistic
    df = DataFrame(result_random_task, columns=['task' ,'errors', 'date_do_task', 'number_of_tasks', 'number_of_right_answer'])
    ...
    # statistic 2
    df_second= DataFrame(time_result_random_task, columns=['task' ,'errors', 'attemps','date_do_task', 'number_of_tasks', 'number_of_right_answer'])
...
def login(update: Update, context: CallbackContext):
    user = update.message.from_user
    logger.info("Пользователь %s: этап - login \ словарь - %s, bl_lis - %s", user.first_name, context.bot_data, black_list)
    ID = update.message.chat_id
    try:
        try:
            key = str(update.effective_user.id) + ' is_admin'
...
def random_task(update, context: CallbackContext):
    ...
    try:
        ...
        if 'after_login' in context.bot_data[key]:
            ...
            context.bot.send_message(chat_id=ID,
                                     text=task[0])
            context.bot_data[key_1] = [task[2], task[1]]
...
def random_task_answer(update, context: CallbackContext):
    user = update.message.from_user
    logger.info("Пользователь %s: этап - random_task_answer \ словарь - %s", user.first_name, context.bot_data)
    ...
    if 'after_login' in context.bot_data[key]:  # проверка на регистрацию
        ...
        try:
            ...
            task = (context.bot_data[key_1][1])  # задание юнит
            ...
            if len(right_answer) == len(answer):  # проверка на недостаток или избыток ответов
                ...
                cursor.execute(
                    """
                            INSERT INTO random_task(task, errors, date_do_task, fk_random_task_user, number_of_tasks, number_of_right_answer) VALUES
                            (%s, %s, %s, %s, %s, %s);
                    """,
                    (task, ' '.join(errors), date, int(user_id), len_answer, right_answer_number))
                print("[INFO] Data was succefully inserted")
                return ConversationHandler.END
            else:
                context.bot.send_message(chat_id=ID,
...
def your_dictionary(update, context: CallbackContext):
    ...
    try:
        ...
        print("[INFO] Error while working with PostgreSQL", _ex)
        context.bot.send_message(chat_id=ID,
                                 text="Скорее всего ваш словарь пуст, пожалуйста запишите туда сначала что нибудь. Если вы уже записывали обратитесь в поодуржку.", reply_markup=markup_key)
        return ConversationHandler.END
    finally:
        if connection:
            cursor.close()
    ...
    try:
        ...
        if 'after_login' in context.bot_data[key]:
            ...
            context.job_queue.run_daily(time_for_task,
                                        datetime.time(hour=10, minute=1, tzinfo=pytz.timezone('Europe/Moscow')),
...
def time_for_task(context: CallbackContext):
    ...
    context.bot_data[job] = [task[2], True, task[1]] # передаем функции посреднику ответ
            ...
            try:
                ...
                if len(right_answer) == len(answer):  # проверка на недостаток или избыток ответов
                    ...
                    if context.bot_data[attempt_counter_key] >= 3 or all_right:
                        ...
                        cursor.execute(
                            """
                                    INSERT INTO time_random_task(task, errors, attemps, date_do_task, fk_random_task_user, number_of_tasks, number_of_right_answer) VALUES
                                    (%s, %s, %s, %s, %s, %s,%s);
                            """,
                            (task, ' '.join(errors), context.bot_data[attempt_counter_key], date,
                             int(user_id), len_answer, right_answer_number))
                        print("[INFO] Data was succefully inserted")
                        context.bot_data[attempt_counter_key] = 0
...
if __name__ == '__main__':
    ...
    conv_handler_2 = ConversationHandler(
        ...
        states={
            RANDOM_TASK_ANSWER: [MessageHandler(Filters.text & (~ Filters.command), random_task_answer, run_async=True)],
    ...
    conv_handler_admin = ConversationHandler(
        ...
        states={
            ADMINISTRATOR_FIRST: [MessageHandler(Filters.text & (~ Filters.command), administrator_first, run_async=True)]
    ...
    dispatcher.add_handler(conv_handler_admin_main)
