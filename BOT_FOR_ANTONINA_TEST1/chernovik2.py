...
def login_data(update: Update, context: CallbackContext):
    ...
    try:
        ...
        if pas and log:
            ...
            return ConversationHandler.END, after_login(update, context) # выдает предупреждение но работает нормально. Тут завенршается этап регистрации и входа
...
def random_task(update, context: CallbackContext):
    ### прекращаем работу timer_task если она есть
    # job_removed = remove_job_if_exists(str(update.effective_user.id), context)
    # print(job_removed)
    markup_key = ReplyKeyboardRemove()
    ID = update.message.chat_id
    task = some_task() # из template_of_tasks берем случаное задание в которой функция с рандомом
    try:
        key = str(update.effective_user.id) # ключ во всех функция это id пользователя, по этому ключю context содержит всю информацию о пользоваиеле
        print('-------',context.bot_data[key])
    except KeyError:
        context.bot.send_message(chat_id=ID,
...
def random_task_answer(update, context: CallbackContext):
    ...
    if 'after_login' in context.bot_data[key]:
        ...
        try:
            ...
            # if context.bot_data[key_1][1]:
            #     print('запуск по новой')
            #     context.job_queue.run_daily(time_for_task,
            #                                 datetime.time(hour=20, minute=43, tzinfo=pytz.timezone('Europe/Moscow')),
...
def dictionary_word(update, context: CallbackContext):
    ...
    ID = update.message.chat_id
    if 'after_login' in context.bot_data[key]:
        words = update.message.text.partition(' ') # сделать строгий пррием ответа, пока делит сообщение на три части разделяя пробелом(одна из частей пробел)
        word = words[0]
        translate = words[2]
...
def timer_task(update, context: CallbackContext):
    ...
    try:
        ...
        if 'after_login' in context.bot_data[key]:
            ...
            context.bot.send_message(chat_id=ID,
                                             text="задание в пути")
            context.job_queue.run_daily(time_for_task,datetime.time(hour=00, minute=32, tzinfo=pytz.timezone('Europe/Moscow')),days=(0, 1, 2, 3, 4, 5, 6), context=[ID, str(ID)+' time'], name=str(update.effective_user.id)) # вызов функции с заданием
...
def time_for_task(context: CallbackContext):
    ...
    context.bot.send_message(chat_id=ID, text=task[0]) # отправляем задание
    context.bot_data[job] = task[1] # передаем функции посреднику ответ
...
if __name__ == '__main__':
    ...
    conv_handler_2 = ConversationHandler(
        ...
        fallbacks=[CommandHandler('unset', unset)]
